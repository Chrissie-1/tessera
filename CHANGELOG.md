...
- **FastAPI dev wrapper** — `GET /health` and unary `POST /v1/completions` for
  exercising the worker without the gateway.
- Backend selected at startup by `TESSERA_BACKEND`; configuration resolved from
  the environment into a frozen `WorkerConfig`.
...
- CPU Docker images for both services and a `docker compose` stack.
- Makefile covering venv, install, proto, test, lint, fmt, run, and docker.
- 159 Python tests and 13 Rust tests, including equivalence tests asserting the
...
- GPT-2-family model configs only — `PagedEngine` reads `config.n_layer` and
  position limits come from `n_positions`.
- `docker compose --scale worker=N` does not work: the worker publishes a fixed
  host port and the gateway's endpoint list is static.
